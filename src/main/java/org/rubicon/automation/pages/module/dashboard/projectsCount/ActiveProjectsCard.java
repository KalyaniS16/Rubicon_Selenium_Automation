...
public class ActiveProjectsCard extends BasePage {
    ...
    /**
     * Locators for Dashboard Active Projects card
     */
    private final By cardTitle = By.xpath("//div[normalize-space()='Active Projects']");
    private final By cardContainer = By.xpath(
            "//div[contains(@class,'dashboard-card')]"
                    + "//div[normalize-space()='Active Projects']"
    ...
    /**
     * Method to verify Dashboard Active Projects card is visible
     */
    public boolean isCardVisible() {
        try {
            wait.until(ExpectedConditions.visibilityOfElementLocated(cardTitle));
            LOG.info("Active Projects card is visible");
            return true;
        } catch (Exception e) {
            LOG.warn("Active Projects card is not visible: {}", e.getMessage());
            return false;
        }
    }

    /**
     * Extract integer count from the dashboard Active Projects card count element.
     * Waits until the async count text is populated before parsing.
     */
    public int getDashboardActiveProjectsCardCount() {
        try {
            ...
            LOG.info("Dashboard Active Projects card count text: {}", text);
    ...
    /**
     * Method to click on the Dashboard Active Projects card
     */
    public void clickActiveProjectsCard() {
        try {
            WebElement card = wait.until(ExpectedConditions.elementToBeClickable(cardContainer));
            card.click();
            LOG.info("Clicked on Dashboard Active Projects card");
        } catch (Exception e) {
            LOG.error("Failed to click on card: {}", e.getMessage());
            throw new RuntimeException("Could not click on Dashboard Active Projects card", e);
        }
    }

    // Locator for Active Projects page title that contains the count, e.g. "Active Projects (1)"
    private final By activeProjectsTitle = By.xpath("//div[contains(@class,'title-ui-design') and contains(normalize-space(),'Active Projects')]");

    /**
     * Extract integer count from the Active Projects page title (e.g. "Active Projects (1)").
     * Waits until the title includes (N) before parsing.
     */
    public int getInnerCardActiveProjectsTitleCount() {
        try {
            WebElement titleEl = wait.until(ExpectedConditions.visibilityOfElementLocated(activeProjectsTitle));
            wait.until(d -> {
            ...
            LOG.info("Active Projects title text: {}", text);
            cachedPageCount = extractFirstInteger(text);
            return cachedPageCount;
        } catch (Exception e) {
            LOG.error("Failed to read Active Projects title count: {}", e.getMessage());
    ...
    public void verifyActiveProjectsCountMatches() {
        ...
        int activeProjectsCount = cachedPageCount != null
                ? cachedPageCount
                : getInnerCardActiveProjectsTitleCount();

        if (cachedDashboardCount.equals(activeProjectsCount)) {
            LOG.info("Active Projects count matches: {}", cachedDashboardCount);
        } else {
            throw new RuntimeException("Active Projects count mismatch - dashboard: "
    ...
    // Export the ActiveProjects
    public void exportActiveProjects() {
        try {
